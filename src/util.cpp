...
const char * const BITCOIN_CONF_FILENAME = "havercoin.conf";
const char * const BITCOIN_PID_FILENAME = "havercoin.pid";
...
#else
    const char* pszModule = "havercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "havercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/havercoin";
#else
    // Unix
    return pathRet / ".havercoin";
#endif
#endif
}
